 ...
 * performance and about Mill, updates its self-model, and sends a brief candid
...
export async function runSelfReflection(bot: TelegramBot) {
    ...
    try {
        const [self, habits, tasks, recentDecisions, history, recentReflections] = await Promise.all([
            getSelfState(),
            getAllHabits(),
            getPendingTasks(),
        ...
        // Skip if nothing meaningful to reflect on yet (brand new agent).
        ...
        // Send the candid self-check-in to Mill (if there's a message).
